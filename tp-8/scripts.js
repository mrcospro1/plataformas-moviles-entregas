...
function sumarLista(listaDeNumeros) {
    return listaDeNumeros.reduce((total,numero) => total + numero, 0);
}
...
function calcularPromedio(listaDeNumeros) {
    return (listaDeNumeros.reduce((total,numero) => total + numero, 0)) / (listaDeNumeros.length);
}
...
function crearListaDeNumeros(inicio, fin) {
    return Array.from({length: Math.abs(fin-inicio) + 1}, (_,i) => (inicio<fin ? inicio + i : inicio - i));
}
...
function encontrarNumeroMayor(listaDeNumeros) {
    ...
    for(let i=1;i<listaDeNumeros.length;i++){
        if (max < listaDeNumeros[i]) max = listaDeNumeros[i];
    }
    return max;
}
...
function encontrarPalabraMasCorta(listaDePalabras) {
        if (listaDePalabras.length === 0) return 0;
    let max = [listaDePalabras[0]];

    for(let i=1;i<listaDePalabras.length;i++){
        if (max[0].length > listaDePalabras[i].length) {
            max.length = 0;
            max[0] = listaDePalabras[i];
        }else if (max[0].length === listaDePalabras[i].length){
            max.push(listaDePalabras[i]);
        }
    }
    return max;
}
...
function contarAprobados(listaDeNotas) {
    return listaDeNotas.reduce((contador,nota) => contador + (nota >= 7 ? 1 : 0),0);
}
 ...
 */
function filtrarSoloTruthy(listaDePalabras) {
     const soloTruthy = listaDePalabras.filter(Boolean);

    if (soloTruthy.length === 0) return "Nada válido para mostrar.";

    if (soloTruthy.length === 1) {
        return soloTruthy[0][0].toUpperCase() + soloTruthy[0].slice(1) + ".";
    }

    const primero = soloTruthy[0][0].toUpperCase() + soloTruthy[0].slice(1);
    const medio = soloTruthy.slice(1, -1).join(", ");
    const ultimo = soloTruthy.at(-1);
    
    if (medio) {
        return `${primero}, ${medio} y ${ultimo}.`;
    } else {
        return `${primero} y ${ultimo}.`;
    }
}
...
function enumerarLista(listaDePalabras) {
            return (listaDePalabras[0][0].toUpperCase() + listaDePalabras[0].slice(1) + ", ") + (listaDePalabras.slice(1,listaDePalabras.length - 1).join(", ") + (" y " + listaDePalabras.at(-1) + "."));
}
console.log("enumerar lista ", enumerarLista(["Han", "Leia", "Luke", "Yoda"]))
